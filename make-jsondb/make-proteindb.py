...
def main():
	...
	for id1 in db_obj:
            if "idmapping" in  db_obj[id1]:
                ...
                if main_key in db_obj[id1]:
                    ...
                    species.append({
                            ...
                            ,"taxid":tax_id
                            ,"evidence":[{"database":"UniProt", "id":uniprotkb_ac,"url":url}]
                        }
                    )
                    ...
                    for obj in db_obj[id1][main_key]:
                        ...
                        if False not in cond_list:
                            ...
                            if combo_id not in glycosylation_dict:
                                ...
                                uckb_url = url_dict["uckb"] % (uckb_id)
                ...
                if main_key in db_obj[id1]:
                    for obj in db_obj[id1][main_key]:
                        ...
                        if False not in cond_list:
                            ...
                            ev_obj = {"database":"UniProt", "id":id1, "url":uniprot_url}
                            ev_detail = "%s,%s,%s" % (combo_id,"UniProt", id1)
                            #If this combo has not been populated 
                            if combo_id not in glycosylation_dict:
                                glycosylation_dict[combo_id] = {
                                    "glytoucan_ac":glytoucan_ac
                                    ...
                                    ,"position":pos
                                    ,"residue":amino_acid
                                    ,"evidence":[{"database":"UniProt", "id":id1, "url":uniprot_url}]
                                }
                            elif ev_detail not in seen_glyco_ev:
                                #In case this combo has been populated, add UniProt evidence
                                glycosylation_dict[combo_id]["evidence"].append(ev_obj)
                                seen_glyco_ev[ev_detail] = True
                ...
                if main_key in db_obj[id1]:
                    for obj in db_obj[id1][main_key]:
                        ...
                        if False not in cond_list:
                            ...
                            combo_id = "%s,%s,%s,%s" %(pos,glytoucan_ac,glycosylation_type,amino_acid)
                            pdb_id = obj["pdb_id"][0]
                            pdb_url = url_dict["pdb"] % (pdb_id)
                            ev_obj = {"database":"PDB", "id":pdb_id, "url":pdb_url}
		...
		function_list = []
                ...
                dblabel_dict = {"refseq":"RefSeq", "uniprotkb":"UniProt"}
                if main_key in db_obj[id1]:
                    for obj in db_obj[id1][main_key]:
		...
		#Extract from "crossref"
                ...
                if main_key in db_obj[id1]:
                    for obj in db_obj[id1][main_key]:
                        ...
                        #    if f[-3:] == "_id":
                        #        db_name = f[:-3]
                        #        if db_name in url_dict:
                        #            for db_id in obj[f]:
                        #                db_url = url_dict[db_name] % (db_id)
                        #                crossref.append({"database":db_name, "id":db_id, "url":db_url})
                db_name, db_id, db_url = "UniProt", uniprotkb_ac, url_dict["uniprotkb"] % (uniprotkb_ac)
                crossref.append({"database":db_name, "id":db_id, "url":db_url})
                ...
                if main_key in db_obj[id1]:
                    for obj in db_obj[id1][main_key]:
                        do_id = obj["do_id"][0]
                        do_name = obj["do_name"][0]
                        if do_id not in seen_doid:
                            continue
	...
	for obj in out_obj_list:
        ...
        print "Done! created %s records" % (record_count)

...
def main():
	...
	db_obj = {}
        ...
        file_list1 = glob.glob(data_dir + "human_glycan_*.csv")
        file_list2 = glob.glob(data_dir + "mouse_glycan_*.csv")
        file_list3 = glob.glob(data_dir + "human_proteoform_*unicarbkb_glytoucan*.csv")
        file_list4 = glob.glob(data_dir + "mouse_proteoform_*unicarbkb_glytoucan*.csv")
        file_list5 = glob.glob(data_dir + "*_protein_recnames.csv")
        exclude_list = [
                "human_glycan_composition.csv"
                ,"mouse_glycan_composition.csv"
        ]
	...
	glyid2speciesindex = {}
        for inFile in file_list1 + file_list2 + file_list3 + file_list4:
                fileName = os.path.basename(inFile)
                if fileName in exclude_list:
                    continue
	...
	for id1 in db_obj:
		...
		out_obj = {"glytoucan_ac":id1}
                ...
                #Extract sequences
                out_obj["iupac"], out_obj["wurcs"], out_obj["glycoct"] = "", "", ""
                if len(db_obj[id1]["sequences"][0]["iupac"]) > 0:
			out_obj["iupac"] = db_obj[id1]["sequences"][0]["iupac"][0]
	...
	for obj in out_obj_list:
		cond_list= []
                ...
                #cond_list.append(len(obj["glycosylation"]) > 0)
		#cond_list = [True]
